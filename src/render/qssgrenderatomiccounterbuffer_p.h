...
#ifndef QSSG_RENDER_RENDER_ATOMIC_COUNTER_BUFFER_H
#define QSSG_RENDER_RENDER_ATOMIC_COUNTER_BUFFER_H
...
class Q_QUICK3DRENDER_EXPORT QSSGRenderAtomicCounterBuffer : public QSSGRenderDataBuffer
{
public:
    /**
     * @brief constructor
     ...
     * @return No return.
     */
    QSSGRenderAtomicCounterBuffer(const QSSGRef<QSSGRenderContext> &context,
                                    const QByteArray &bufferName,
                                    QSSGRenderBufferUsageType usageType,
                                    QSSGByteView data);

    ///< destructor
    virtual ~QSSGRenderAtomicCounterBuffer() override;

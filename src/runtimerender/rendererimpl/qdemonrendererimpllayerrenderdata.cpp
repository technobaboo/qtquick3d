                                          ...
                                          const QDemonRenderCamera &inCamera)
{
    ...
    if (inObject.renderableFlags.isDefaultMaterialMeshSubset())
        static_cast<QDemonSubsetRenderableBase &>(inObject).renderShadowMapPass(inCameraProps, inData.globalLights[lightIndex], inCamera, pEntry);
    else if (inObject.renderableFlags.isCustomMaterialMeshSubset()) {
        static_cast<QDemonSubsetRenderableBase &>(inObject).renderShadowMapPass(inCameraProps, inData.globalLights[lightIndex], inCamera, pEntry);
    } else if (inObject.renderableFlags.isPath()) {
        static_cast<QDemonPathRenderable &>(inObject).renderShadowMapPass(inCameraProps, inData.globalLights[lightIndex], inCamera, pEntry);
    }
}
                                      ...
                                      const QDemonRenderCamera &inCamera)
{
    if (inObject.renderableFlags.isDefaultMaterialMeshSubset())
        ...
        static_cast<QDemonCustomMaterialRenderable &>(inObject).renderDepthPass(inCameraProps, inData.layer, inData.globalLights, inCamera, nullptr);
    } else if (inObject.renderableFlags.isPath()) {
        static_cast<QDemonPathRenderable &>(inObject).renderDepthPass(inCameraProps, inData.layer, inData.globalLights, inCamera, nullptr);
    } else {
        Q_ASSERT(false);
    }
}
                             ...
                             const QDemonRenderCamera &inCamera)
{
    if (inObject.renderableFlags.isDefaultMaterialMeshSubset())
        ...
        if (!inData.layer.lightProbe && theObject.material.m_iblProbe)
            inData.setShaderFeature(QDemonShaderDefines::lightProbe(), theObject.material.m_iblProbe->m_textureData.m_texture != nullptr);
        else if (inData.layer.lightProbe)
            inData.setShaderFeature(QDemonShaderDefines::lightProbe(), inData.layer.lightProbe->m_textureData.m_texture != nullptr);
                                          ...
                                          QDemonResourceFrameBuffer *theFB)
{
    ...
    for (const auto &theObject : theOpaqueObjects) {
        ...
        setShaderFeature(QDemonShaderDefines::cgLighting(), globalLights.empty() == false);
...
inline bool anyCompletelyNonTransparentObjects(const QDemonLayerRenderPreparationData::TRenderableObjectList &inObjects)
{
    ...
    if (!thePrepResult.flags.shouldRenderToTexture()) {
...

        {
...
#endif
            {
                ...
                theContext->setDepthTestEnabled(false);
#ifdef ADVANCED_BLEND_SW_FALLBACK
                QDemonRef<QDemonRenderTexture2D> screenTexture = renderer->layerBlendTexture();
                QDemonRef<QDemonRenderFrameBuffer> blendFB = renderer->blendFrameBuffer();
...
{
    QDemonStackPerfTimer __perfTimer(renderer->demonContext()->performanceTimer(), Q_FUNC_INFO);
    Q_ASSERT(camera);
    if (!camera)
        return;

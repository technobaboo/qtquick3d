...
struct QDemonSubsetMaterialVertexPipeline : public QDemonVertexPipelineImpl
{
    ...
    QDemonSubsetMaterialVertexPipeline(QDemonRendererImpl &inRenderer, QDemonSubsetRenderable &inRenderable, bool inWireframeRequested)
        : QDemonVertexPipelineImpl(inRenderer.demonContext()->defaultMaterialShaderGenerator(),
                                   inRenderer.demonContext()->shaderProgramGenerator(),
                                   false)
        , renderer(inRenderer)
    ...
    {
        if (tessMode == TessModeValues::NoTess || programGenerator()->getStage(QDemonShaderGeneratorStage::TessControl) == nullptr) {
            return;
        }
        ...
        setupTessIncludes(QDemonShaderGeneratorStage::TessControl, tessMode);
    ...
    {
        if (tessMode == TessModeValues::NoTess || programGenerator()->getStage(QDemonShaderGeneratorStage::TessEval) == nullptr) {
            return;
        }
        ...
        if (tessMode == TessModeValues::TessLinear)
            renderer.demonContext()->defaultMaterialShaderGenerator()->addDisplacementImageUniforms(tessEvalShader,
                                                                                                          m_displacementIdx,
                                                                                                          m_displacementImage);
        ...
        if (tessMode == TessModeValues::TessNPatch) {
            ...
            if (tessMode == TessModeValues::TessLinear && m_displacementImage) {
                QDemonDefaultMaterialShaderGeneratorInterface::ImageVariableNames
                        theNames = renderer.demonContext()->defaultMaterialShaderGenerator()->getImageVariableNames(m_displacementIdx);
                                                                        ...
                                                                        const TShaderFeatureSet &inFeatureSet)
{
    ...
    QDemonRef<QDemonShaderCacheInterface> theCache = m_demonContext->shaderCache();
    ...
    return m_demonContext->defaultMaterialShaderGenerator()->generateShader(inRenderable.material,
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_paraboloidDepthShader;
    ...
    if (theDepthShader.isNull()) {
        ...
        if (!depthShaderProgram) {
            ...
            QDemonShaderStageGeneratorInterface &vertexShader(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::Vertex));
            QDemonShaderStageGeneratorInterface &fragmentShader(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::Fragment));
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_paraboloidDepthTessLinearShader;
    ...
    if (theDepthShader.isNull()) {
        ...
        if (!depthShaderProgram) {
            getProgramGenerator()->beginProgram(
                    QDemonShaderGeneratorStageFlags(QDemonShaderGeneratorStage::Vertex | QDemonShaderGeneratorStage::TessControl
                                               | QDemonShaderGeneratorStage::TessEval | QDemonShaderGeneratorStage::Fragment));
            QDemonShaderStageGeneratorInterface &vertexShader(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::Vertex));
            QDemonShaderStageGeneratorInterface &tessCtrlShader(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::TessControl));
            QDemonShaderStageGeneratorInterface &tessEvalShader(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::TessEval));
            QDemonShaderStageGeneratorInterface &fragmentShader(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::Fragment));
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_paraboloidDepthTessPhongShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_paraboloidDepthTessNPatchShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_cubemapDepthShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_cubemapDepthTessLinearShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_cubemapDepthTessPhongShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_cubemapDepthTessNPatchShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_orthographicDepthShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_orthographicDepthTessLinearShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_orthographicDepthTessPhongShader;
...
{
    QDemonRef<QDemonRenderableDepthPrepassShader> &theDepthShader = m_orthographicDepthTessNPatchShader;
...
QDemonRef<QDemonRenderableDepthPrepassShader> QDemonRendererImpl::getDepthPrepassShader(bool inDisplaced)
{
    ...
    if (theDepthPrePassShader.isNull()) {
        ...
        if (!depthShaderProgram) {
            ...
            if (inDisplaced) {
                demonContext()->defaultMaterialShaderGenerator()->addDisplacementMappingForDepthPass(vertexShader);
            } else {
                vertexShader.append("\tgl_Position = model_view_projection * vec4(attr_pos, 1.0);");
            }
            vertexShader.append("}");
...
QDemonRef<QDemonDefaultAoPassShader> QDemonRendererImpl::getDefaultAoPassShader(TShaderFeatureSet inFeatureSet)
{
    if (m_defaultAoPassShader.isNull()) {
        ...
        if (!aoPassShaderProgram) {
            getProgramGenerator()->beginProgram();
            QDemonShaderStageGeneratorInterface &theVertexGenerator(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::Vertex));
            QDemonShaderStageGeneratorInterface &theFragmentGenerator(*getProgramGenerator()->getStage(QDemonShaderGeneratorStage::Fragment));

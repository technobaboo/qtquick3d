...
class Q_QUICK3DRUNTIMERENDER_EXPORT QSSGEffectSystem
{
    ...
    typedef QHash<QByteArray, QSSGRef<QSSGEffectClass>> TEffectClassMap;
...
public:
    ...
    const QSSGRef<QSSGEffectClass> getEffectClass(const QByteArray &inStr) const;
    ...
    // assumed to be.
    void setEffectPropertyDefaultValue(QString inName, QString inPropName, QSSGByteView inDefaultData);

    void setEffectPropertyEnumNames(QString inName, QString inPropName, QSSGDataView<QString> inNames);
    ...
    bool registerEffect(const QByteArray &inName /*, QSSGDataView<dynamic::QSSGPropertyDeclaration> inProperties*/);
    ...
    QSSGRenderEffect *createEffectInstance(const QByteArray &inEffectName);
    ...
    void allocateImage(QSSGRenderEffect &inEffect,
                       const dynamic::QSSGAllocateImage &inCommand,
                       quint32 inFinalWidth,
                       quint32 inFinalHeight);
    ...
    QSSGRef<QSSGEffectShader> bindShader(const QByteArray &inEffectId, const dynamic::QSSGBindShader &inCommand);
    ...
    void applyValue(QSSGRenderEffect *inEffect,
                    const QSSGRef<QSSGEffectClass> &inClass,
                    const QSSGRef<QSSGRenderShaderProgram> &inShader,
                    const dynamic::QSSGApplyValue &inCommand);

...
QDemonMaterialSystem::~QDemonMaterialSystem()
{
    ...
    for (quint32 idx = 0; idx < d->allocatedImages.size(); ++idx) {
...
void QDemonMaterialSystem::releaseBuffer(quint32 inIdx)
{
...
quint32 QDemonMaterialSystem::findBuffer(const QString &inName)
{
    for (quint32 idx = 0, end = d->allocatedBuffers.size(); idx < end; ++idx)
        if (d->allocatedBuffers[idx].name == inName)
            return idx;
    return d->allocatedBuffers.size();
}

quint32 QDemonMaterialSystem::findAllocatedImage(const QString &inName)
{
    for (quint32 idx = 0, end = d->allocatedImages.size(); idx < end; ++idx)
...
void QDemonMaterialSystem::setPropertyTextureSettings(const QString &inName, const QString &inPropName, const QString &inPropPath, QDemonRenderTextureTypeValue::Enum inTexType, QDemonRenderTextureCoordOp::Enum inCoordOp, QDemonRenderTextureMagnifyingOp::Enum inMagFilterOp, QDemonRenderTextureMinifyingOp::Enum inMinFilterOp)
{
...
void QDemonMaterialSystem::doApplyInstanceValue(QDemonRenderCustomMaterial &, quint8 *inDataPtr, const QString &inPropertyName, QDemonRenderShaderDataTypes::Enum inPropertyType, const QDemonRef<QDemonRenderShaderProgram> &inShader, const dynamic::QDemonPropertyDefinition &inDefinition)
{
...
QDemonRef<QDemonRenderFrameBuffer> QDemonMaterialSystem::bindBuffer(const QDemonRenderCustomMaterial &inMaterial, const dynamic::QDemonBindBuffer &inCommand, bool &outClearTarget, QVector2D &outDestSize)
{
    ...
    if (theTexture) {
        ...
        d->context->getRenderContext()->setViewport(QRect(0, 0, (quint32)theDetails.width, (quint32)theDetails.height));
...
void QDemonMaterialSystem::doRenderCustomMaterial(QDemonCustomMaterialRenderContext &inRenderContext, const QDemonRenderCustomMaterial &inMaterial, QDemonMaterialClass &inClass, const QDemonRef<QDemonRenderFrameBuffer> &inTarget, const TShaderFeatureSet &inFeatureSet)
{
    ...
    for (quint32 idx = 0; idx < d->allocatedBuffers.size(); ++idx) {
